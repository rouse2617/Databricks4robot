...
psql_exec() {
  if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx 'local-postgres-1'; then
    docker exec -i local-postgres-1 psql -U postgres -d data4cyber -v ON_ERROR_STOP=1 "$@"
  else
    export PGPASSWORD="${PGPASSWORD:-postgres}"
    psql -h "${PGHOST:-127.0.0.1}" -p "${PGPORT:-5432}" -U "${PGUSER:-postgres}" \
      -d "${PGDATABASE:-data4cyber}" -v ON_ERROR_STOP=1 "$@"
  fi
}
...
psql_exec_file() {
  ...
  if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx 'local-postgres-1'; then
    ...
    docker exec -i local-postgres-1 psql -U postgres -d data4cyber -v ON_ERROR_STOP=1 -f - < "$file"
  else
    export PGPASSWORD="${PGPASSWORD:-postgres}"
    psql -h "${PGHOST:-127.0.0.1}" -p "${PGPORT:-5432}" -U "${PGUSER:-postgres}" \
      -d "${PGDATABASE:-data4cyber}" -v ON_ERROR_STOP=1 -f "$file"
  fi
}
