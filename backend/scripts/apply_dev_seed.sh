...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx 'local-postgres-1'; then
  ...
  cat "$SEED" | docker exec -i local-postgres-1 psql -U postgres -d cyber_databrew_dev -v ON_ERROR_STOP=1
else
  export PGPASSWORD="${PGPASSWORD:-postgres}"
  psql -h "${PGHOST:-127.0.0.1}" -p "${PGPORT:-5432}" -U "${PGUSER:-postgres}" \
    -d "${PGDATABASE:-cyber_databrew_dev}" -v ON_ERROR_STOP=1 -f "$SEED"
fi
