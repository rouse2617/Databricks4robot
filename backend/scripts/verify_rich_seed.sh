...
echo "=== healthz ==="
curl -sS -o /dev/null -w "%{http_code}\n" -H "X-Grace-Token: ${TOKEN}" "${BASE}/healthz"

echo "=== GET /api/v1/metrics/registry (count items) ==="
curl -sS -H "X-Grace-Token: ${TOKEN}" "${BASE}/api/v1/metrics/registry" \
...
curl -sS -H "X-Grace-Token: ${TOKEN}" -H "Content-Type: application/json" \
