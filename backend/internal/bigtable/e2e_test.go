...
func setupE2E(t *testing.T) *e2eEnv {
	...
	queryHandler := queryH.New(uc, nil, nil, nil)
	...
	r := gin.New()
	r.Use(gin.Recovery())
	routes.RegisterAll(r, cfg, assetHandler, mcapHandler, deliveryHandler, algoHandler, nil, nil, nil, nil, nil, nil, queryHandler)
...
func TestE2E_AssetCRUD(t *testing.T) {
	...
	// 5. POST /api/v1/queries/run → list (should exclude archived asset).
	...
	resp = env.doJSON(t, "POST", "/api/v1/queries/run", map[string]interface{}{
		"schema_version": "v1",
		"mode":           "structured",
	...
	if resp.StatusCode != http.StatusOK {
		body := readJSON(t, resp)
		t.Fatalf("POST /queries/run: expected 200, got %d: %v", resp.StatusCode, body)
	}
	listResp := readJSON(t, resp)
	items, ok := listResp["items"].([]interface{})
	if !ok {
		t.Fatalf("POST /queries/run: items not an array: %v", listResp["items"])
	}
	// The first asset was soft-deleted (archived), so only the second should appear.
	for _, item := range items {
		m := item.(map[string]interface{})
		if m["asset_id"] == assetID {
			t.Fatalf("POST /queries/run: archived asset should not appear in list")
		}
	}
}
