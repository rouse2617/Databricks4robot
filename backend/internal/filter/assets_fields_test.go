...
func TestFilterResolve_DurationSecConversion(t *testing.T) {
	...
	sortExpr, _, err := ResolveSortBy("duration_sec", 1)
	if err != nil {
		t.Fatalf("ResolveSortBy(duration_sec) error: %v", err)
	}
	if !strings.Contains(sortExpr.SQL, "duration_ms") {
		t.Errorf("expected sort expression to reference duration_ms, got %q", sortExpr.SQL)
	}
	if strings.Contains(sortExpr.SQL, "#>>") {
		t.Errorf("expected no JSONB extraction in sort expression, got %q", sortExpr.SQL)
	}
}
