...
func ParseFilter(filterStr string) (*Filter, error) {
	...
	// like/ilike:
	// - always treat the raw value as string (even if it looks like a number)
	// - auto-wrap with % wildcards if not already present
	if opName == "like" || opName == "ilike" {
		value = valueStr
		if !strings.Contains(valueStr, "%") && !strings.Contains(valueStr, "_") {
			value = "%" + valueStr + "%"
		}
	}
