...
var SearchableFields = map[string]FieldMeta{
	...
	"failure_mode":       {Type: "virtual"},
	"action.label":       {Type: "string"},
	"action.source_type": {Type: "enum", Values: []string{"human", "algo"}},
	"action.source_name": {Type: "string"},
	"action.confidence":  {Type: "numeric"},
	"action.start_ns":    {Type: "numeric"},
	"action.end_ns":      {Type: "numeric"},
}
...
// Postgres SQL generation for virtual filter fields.
type VirtualFieldHandler interface {
	...
	// MatchBigtable checks in-memory matching (legacy, unused in current Postgres-only runtime).
...
var VirtualFields = map[string]VirtualFieldHandler{
	...
	"failure_mode": failureModeVirtualHandler{},
}
...
// failureModeVirtualHandler filters assets by their latest algo_failed failure_mode
// in asset_events. Supports eq/ne operators only.
type failureModeVirtualHandler struct{}

func (h failureModeVirtualHandler) BuildSQL(value interface{}, paramIdx int) (string, []interface{}, int, error) {
	sql := fmt.Sprintf(
		"EXISTS (SELECT 1 FROM asset_events ae WHERE ae.asset_id = assets.asset_id AND ae.event_type = 'algo_failed' AND ae.event_payload->>'failure_mode' = $%d)",
		paramIdx,
	)
	return sql, []interface{}{value}, paramIdx + 1, nil
}

func (h failureModeVirtualHandler) MatchBigtable(_ map[string]string, _ int, _ interface{}) bool {
	return false
}
