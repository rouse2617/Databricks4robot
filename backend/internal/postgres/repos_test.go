...
func TestListWithFilters_ScanError(t *testing.T) {
	...
	db.queryRow = &fakeRow{values: []any{int64(1)}}
	db.rows = &fakeRows{data: [][]any{{"a1"}}} // too few columns → scan error (need 22)
	_, _, err := repo.ListWithFilters(ctx, "", nil, 1, 20, "")
...
// ---------------------------------------------------------------------------
// Property 6: Real column consistency on Set()
// For any asset mutation, the promoted real columns SHALL contain correct values.
// ---------------------------------------------------------------------------
...
func TestProperty6_RealColumnConsistency(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// Set() now passes 28 positional args.
		if len(capturedArgs) < 28 {
			t.Fatalf("expected 28 args, got %d", len(capturedArgs))
		}
...
// ---------------------------------------------------------------------------
// buildAssetRow builds a fake row slice (22 columns) matching the SELECT in
...
func TestAssetEventRepo_CursorSeq_NoRow(t *testing.T) {
	repo := &AssetEventRepo{c: &Client{db: &fakeDB{queryRow: &fakeRow{err: errNoRows}}}}
	seq, err := repo.CursorSeq(context.Background(), "es_assets")
	if err != nil {
		t.Fatalf("CursorSeq: %v", err)
	}
	if seq != 0 {
...
func TestAssetEventRepo_CursorSeq_Value(t *testing.T) {
	repo := &AssetEventRepo{c: &Client{db: &fakeDB{queryRow: &fakeRow{values: []any{int64(99)}}}}}
	seq, err := repo.CursorSeq(context.Background(), "es_assets")
	if err != nil {
		t.Fatalf("CursorSeq: %v", err)
	}
...
func TestAssetEventRepo_AdvanceCursor_RequiresSink(t *testing.T) {
	repo := &AssetEventRepo{c: &Client{db: &fakeDB{}}}
	if err := repo.AdvanceCursor(context.Background(), "", 1); err == nil {
		t.Fatal("expected error for empty sink")
	}
	if err := repo.AdvanceCursor(context.Background(), "x", -1); err == nil {
