...
func (f *fakeAssetRepo) InsertNew(context.Context, *models.Asset) error         { return nil }
...
func newTestESServer(t *testing.T) (*httptest.Server, *[]string, *[]string) {
	server, bulkIDs, deletedIDs, _ := newTestESServerWithStats(t)
	return server, bulkIDs, deletedIDs
}

func newTestESServerWithStats(t *testing.T) (*httptest.Server, *[]string, *[]string, *int) {
	t.Helper()
	...
	return server, &bulkIDs, &deletedIDs, &bulkRequests
}
...
func TestESConsumer_HandleBatch_UsesConfiguredChunkSize(t *testing.T) {
	server, bulkIDs, _, bulkRequests := newTestESServerWithStats(t)
	defer server.Close()
	...
	err := consumer.HandleBatch(context.Background(), []ChangeEvent{
		{Table: "assets", Op: OperationUpdate, After: map[string]any{"asset_id": "a1"}},
		{Table: "assets", Op: OperationUpdate, After: map[string]any{"asset_id": "a2"}},
	})
	if err != nil {
		t.Fatalf("HandleBatch returned error: %v", err)
	}
	...
	if *bulkRequests != 2 {
		t.Fatalf("expected 2 bulk requests due to chunk size 1, got %d", *bulkRequests)
	}
}
