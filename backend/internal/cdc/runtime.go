...
func (r *Runtime) HandleTopicBatch(ctx context.Context, topic string, events []ChangeEvent) error {
	...
	if !ok {
		CDCUnknownTopicTotal.Inc()
		if !r.Config.StrictTopicRouting {
			slog.Warn("cdc runtime: dropping batch for unknown topic", "topic", topic, "events", len(events))
			return nil
		}
