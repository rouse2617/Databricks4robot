// Package cdc hosts the WAL/CDC runtime and its sink adapters.
//
...
//
//	staging/events_{minSeq}_{maxSeq}_{count}.jsonl
//
...
// StagingDir returns the directory used for staging files.
func (s *BronzeSink) StagingDir() string {
	if s == nil {
		return ""
	}
	return s.stagingDir
...
func (s *BronzeSink) WriteBatch(events []*models.AssetEvent) (string, error) {
	...
	// Deterministic name so retries after crash do not multiply staging files for the same batch.
	filename := fmt.Sprintf("events_%d_%d_%d.jsonl", minSeq, maxSeq, len(events))
	...
	if err := f.Sync(); err != nil {
		f.Close()
		os.Remove(tmpPath)
		return "", fmt.Errorf("bronze sink: sync temp file: %w", err)
	}
