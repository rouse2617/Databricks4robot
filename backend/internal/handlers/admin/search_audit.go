...
func (h *Handler) SearchAudit(c *gin.Context) {
	...
	// Cheap count for PG (non-deleted) — single COUNT(*) via ListWithFilters.
	_, pgCount, err := h.assets.ListWithFilters(ctx, "", nil, 1, 1, filter.OrderByClause{SQL: "asset_id ASC"})
	if err != nil {
	...
	// Per-id diff is O(N) over both stores; only safe for small datasets.
	// For larger datasets fall back to count-only audit to keep this endpoint snappy.
	const fullDiffMax = 5000
	var missing, orphan int64
	var missingSample, orphanSample []string
	const sampleN = 10

	if pgCount <= fullDiffMax && esCount <= fullDiffMax {
		const pageSize = 500
		pgIDs := make([]string, 0, pgCount)
		page := 1
		for {
		...
		// Count-only fallback: gap = |pg - es|, attribute to missing or orphan based on direction.
		if pgCount > esCount {
			missing = pgCount - esCount
		} else if esCount > pgCount {
			orphan = esCount - pgCount
		}
	}
