...
func (h *Handler) TrainingAssets(c *gin.Context) {
	...
	rows, err := h.trino.QueryRows(c.Request.Context(), `
		SELECT dataset_snapshot_id, asset_id, mcap_file_id, segment_locator, lifecycle_state, asset_type, algo_key, algo_status, output_uri, quality, created_at
...
func parseAlgoKey(algoKey string) (name, version string) {
	algoKey = strings.TrimSpace(algoKey)
	if algoKey == "" {
		return "", ""
	}
...
func (h *Handler) RecomputeCandidates(c *gin.Context) {
	...
	algoName, algoVersion := parseAlgoKey(algoKey)
	if algoName == "" || algoVersion == "" {
		httpresp.BadRequest(c, httpresp.CodeInvalidArgument, "algo_key must be in the form <algo_name>@<algo_version>", nil)
		return
	}
	rows, err := h.trino.QueryRows(c.Request.Context(), `
		SELECT a.asset_id, a.mcap_file_id, a.lifecycle_state, a.asset_type,
		       algo.algo_name, algo.algo_version, algo.status, algo.updated_at
		FROM `+h.trino.Table("silver_asset_algo_latest")+` algo
		JOIN `+h.trino.Table("silver_assets_current")+` a ON a.asset_id = algo.asset_id
		WHERE algo.algo_name = ?
		  AND algo.algo_version = ?
...
// It compares the Bronze-covered event_seq window in PostgreSQL with
// bronze_asset_events in Iceberg via Trino for a real-time sync health check.
// Falls back to the sync_reconciliation table if available.
func (h *Handler) SyncStatus(c *gin.Context) {
	ctx := c.Request.Context()

	// Try real-time comparison on the seq window currently materialized in Bronze.
	if h.pg != nil && h.trino != nil {
		iceTotal, iceErr := h.trino.BronzeEventCount(ctx)
		iceMinSeq, bronzeMaxSeq, boundsErr := h.trino.BronzeEventSeqBounds(ctx)
		iceMaxSeq, seqErr := h.trino.BronzeMaxEventSeq(ctx)
		pgTotal, pgErr := h.pgAssetEventCountInRange(ctx, iceMinSeq, bronzeMaxSeq)

		if pgErr == nil && iceErr == nil && boundsErr == nil && seqErr == nil {
			data := newRealtimeSyncStatusData(time.Now(), pgTotal, iceTotal, iceMaxSeq)
			observeSyncStatus("realtime", &data)
...
LIMIT 1`
	...
	observeSyncStatus("sync_reconciliation", &resp)
...
func observeSyncStatus(source string, data *SyncStatusData) {
	if data == nil {
		return
	}
	metrics.BackendLakehouseSyncCountDiffRatio.Set(data.CountDiffPct)
	metrics.BackendLakehouseSyncPGTotal.Set(float64(data.PgTotalCount))
	metrics.BackendLakehouseSyncIcebergTotal.Set(float64(data.IcebergTotalCount))
	metrics.BackendLakehouseSyncCheckedAtUnixSeconds.Set(float64(data.CheckedAt.Unix()))
	metrics.BackendLakehouseSyncSourceUp.WithLabelValues("realtime").Set(0)
	metrics.BackendLakehouseSyncSourceUp.WithLabelValues("sync_reconciliation").Set(0)
	metrics.BackendLakehouseSyncSourceUp.WithLabelValues(source).Set(1)
}

// pgAssetEventCountInRange returns the count of asset_events rows in the
// Bronze-covered [minSeq, maxSeq] window. This avoids false alerts when local
// Debezium runs with snapshot.mode=never and historical rows predate the
// connector registration.
func (h *Handler) pgAssetEventCountInRange(ctx context.Context, minSeq, maxSeq int64) (int64, error) {
	if minSeq <= 0 || maxSeq <= 0 || maxSeq < minSeq {
		return 0, nil
	}
	var count int64
	err := h.pg.QueryRow(ctx,
		"SELECT count(*) FROM asset_events WHERE event_seq >= $1 AND event_seq <= $2",
		minSeq, maxSeq,
	).Scan(&count)
	return count, err
}
