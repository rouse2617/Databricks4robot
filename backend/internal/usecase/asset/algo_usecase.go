...
//   - asset_events is the append-only CDC source: every state transition
//     appends one event in the SAME transaction as the projection write, so
//     ES / Iceberg / audit consumers can replay deterministically.
...
//	}
func makePayload(algoKey, algoName, algoVersion, prev, next, runID, reason string, extra map[string]string) []byte {
...
// failureModeRules defines classification rules for algo failure reasons.
// Rules are evaluated in order; the first match wins.
var failureModeRules = []struct {
	mode  string
	match func(reason string) bool
}{
	{"timeout", func(r string) bool {
		return strings.Contains(r, "timeout") || strings.Contains(r, "deadline") || strings.Contains(r, "timed out")
	}},
	{"algo_error", func(r string) bool {
		return strings.Contains(r, "panic") || strings.Contains(r, "OOM") || strings.Contains(r, "nil pointer") || strings.Contains(r, "segmentation fault") || strings.Contains(r, "out of memory") || strings.Contains(r, "oom")
	}},
	{"sensor_fault", func(r string) bool {
		return strings.Contains(r, "sensor") || strings.Contains(r, "corrupt") || strings.Contains(r, "hardware") || strings.Contains(r, "camera") || strings.Contains(r, "imu") || strings.Contains(r, "lidar")
	}},
	{"env_mismatch", func(r string) bool {
		return strings.Contains(r, "env") || strings.Contains(r, "scene") || strings.Contains(r, "lighting") || strings.Contains(r, "weather")
	}},
	{"low_quality", func(r string) bool {
		return strings.Contains(r, "blur") || strings.Contains(r, "occlusion") || strings.Contains(r, "dark") || strings.Contains(r, "resolution") || strings.Contains(r, "noise")
	}},
	{"annotation_drift", func(r string) bool {
		return strings.Contains(r, "annotation") || strings.Contains(r, "label drift") || strings.Contains(r, "inconsistent")
	}},
}

// inferFailureMode classifies an algo failure reason into a failure_mode.
// Returns "" when reason is empty; "unknown" when no rule matches.
func inferFailureMode(reason string) string {
	if reason == "" {
		return ""
	}
	lower := strings.ToLower(reason)
	for _, rule := range failureModeRules {
		if rule.match(lower) {
			return rule.mode
		}
	}
...
func (u *AlgoUsecase) StartAlgo(ctx context.Context, assetID, algoKey string, input StartAlgoInput) error {
	...
	return u.tx.WithTx(ctx, func(txCtx context.Context) error {
		...
		return u.eventRepo.Append(txCtx, repository.AssetEventAppendInput{
			...
			AssetID:      assetID,
			RunID:        runID,
			EventPayload: makePayload(algoKey, algoName, algoVersion, string(curStatus), string(models.AlgoStatusRunning), runID, "", nil),
		})
	})
}
...
func (u *AlgoUsecase) FinishAlgo(ctx context.Context, assetID, algoKey string, input FinishAlgoInput) error {
	...
	err := u.tx.WithTx(ctx, func(txCtx context.Context) error {
		...
		var payloadExtra map[string]string
		if finishStatus == models.AlgoStatusFailed {
			eventType = eventAlgoFailed
			if fm := inferFailureMode(reason); fm != "" {
				payloadExtra = map[string]string{"failure_mode": fm}
			}
		}
...
func (u *AlgoUsecase) ResetAlgo(ctx context.Context, assetID, algoKey string) error {
	...
	return u.tx.WithTx(ctx, func(txCtx context.Context) error {
		...
		return u.eventRepo.Append(txCtx, repository.AssetEventAppendInput{
			...
			AssetID:      assetID,
			EventPayload: makePayload(algoKey, algoName, algoVersion, string(curStatus), string(models.AlgoStatusPending), "", "", nil),
		})
	})
}
	...
	for downName, def := range all {
		...
		for _, ver := range def.Versions {
			...
			if err := u.eventRepo.Append(ctx, repository.AssetEventAppendInput{
				...
				AssetID:      assetID,
				EventPayload: makePayload(downKey, downName, ver, string(models.AlgoStatusBlocked), string(models.AlgoStatusPending), "", "", nil),
			}); err != nil {
				return err
			}
		}
