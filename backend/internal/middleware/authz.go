...
// CtxKeyScopes holds the authenticated principal's scopes ([]string).
const CtxKeyScopes = "user_scopes"

// ScopesForRole maps a human/JWT role to a scope set, so web users and
// machine (API key) callers are authorized through the SAME scope check.
...
// Must run after Authenticate (which populates CtxKeyScopes).
...
// Authenticate is the unified authentication middleware. It accepts, in a
// single normalized credential (X-Databrew-Token header, Authorization Bearer,
// or databrew_session cookie), any of:
//
//   - API key  (dbk_<prefix>_<secret>) → looked up in the store, scoped
//   - JWT       (web email-login / session)     → scopes derived from role
//   - static token (legacy SDK, admin)          → all scopes
//
// All paths populate CtxKeyEmail / CtxKeyRole / CtxKeyScopes so downstream
// RequireScope works uniformly. keys may be nil (API key path disabled).
func Authenticate(staticToken, jwtSecret string, keys repository.APIKeyRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if auth.IsAPIKey(cred) {
			...
			c.Set(CtxKeyEmail, "apikey:"+apiKeyLabel(k.Name, k.Owner, k.KeyPrefix))
			c.Set(CtxKeyRole, "service")
			c.Set(CtxKeyScopes, k.Scopes)
			id := k.ID
		...
		if staticToken != "" && subtle.ConstantTimeCompare([]byte(cred), []byte(staticToken)) == 1 {
			...
			c.Set(CtxKeyScopes, ScopesForRole("admin"))
			c.Next()
			return
		}
		...
		if claims, err := auth.VerifyToken(jwtSecret, cred); err == nil {
			...
			c.Set(CtxKeyScopes, ScopesForRole(claims.Role))
			c.Next()
			return
		}
