-- Reconciliation (idempotent): add the databrew_runs family that the code
-- uses (DatabrewRun / ComponentRelease / rag) but the dev-snapshot baseline
-- lacks. dev creates them; prod already has them (IF NOT EXISTS -> no-op),
-- so this converges both envs. Reused verbatim from the pre-reset 055.
-- Depends on pipeline_runs (present in the baseline).
