-- 001_init.sql — Target DDL for cyber_databrew_dev PostgreSQL schema.
...
CREATE TABLE assets (
    ...
    mcap_file_id        TEXT            NOT NULL CHECK (mcap_file_id ~ '^[0-9A-Za-z]{8}$'),
...
ALTER TABLE assets ADD CONSTRAINT fk_assets_mcap
...
CREATE INDEX idx_assets_algo_inputs_uris_gin
    ON assets USING GIN (algo_inputs_uris);
CREATE INDEX idx_assets_annot_inputs_uris_gin
