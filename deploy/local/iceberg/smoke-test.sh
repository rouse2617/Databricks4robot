...
#
# Prerequisites:
#   docker compose --profile lakehouse up -d   (run from deploy/local)
#   (or: make iceberg-up)
#
# Usage:
...
ICEBERG_REST_URI="${ICEBERG_REST_URI:-http://localhost:8183}"
...
TRINO_CMD="docker compose --profile lakehouse exec -T trino trino --server http://localhost:8080 --catalog iceberg --schema robot"
