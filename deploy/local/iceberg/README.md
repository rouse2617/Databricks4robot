...
- `bigquery`: SQL query engine for Iceberg analytical reads.
...
- BigQuery: http://localhost:8082

The Spark container mounts `deploy/local/iceberg/notebooks` into the notebook workspace.

## Smoke Test

Run the automated smoke test to verify PyIceberg + BigQuery connectivity:
...
After running the MVP builder, query Iceberg through BigQuery:
...
```bash
cd deploy/local
docker compose --profile lakehouse exec -T bigquery \
  bigquery --server http://localhost:8080 --catalog iceberg --schema robot \
  --execute "SELECT count(*) FROM silver_assets_current"
```

The backend lakehouse APIs use BigQuery for typed analytical reads:
