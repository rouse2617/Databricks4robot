...
POSTGRES_URL = os.getenv("POSTGRES_URL", "jdbc:postgresql://postgres:5432/cyber_databrew_dev")
POSTGRES_PROPERTIES = {
    "user": "postgres",
    "password": "postgres",
...
spark.sql(
    ...
    SELECT
      ...
      a.lifecycle_state,
      a.asset_type,
      algo.algo_name || '@' || algo.algo_version AS algo_key,
    ...
    WHERE algo.algo_name = 'hand_tracking'
      AND algo.algo_version = '1.2.0'
      AND algo.status = 'ok'
...
spark.sql(
    """
    SELECT algo_name || '@' || algo_version AS algo_key, status, count(*) AS asset_count
    FROM demo.robot.silver_asset_algo_latest
    GROUP BY algo_name, algo_version, status
    ...
    "questions": {
        "training_assets": {
            ...
            "sample": collect_dicts(
                """
                SELECT dataset_snapshot_id, asset_id, mcap_file_id, lifecycle_state, asset_type, algo_key, algo_status, quality
                FROM demo.robot.gold_dataset_snapshot_items
                ORDER BY asset_id
                """,
                limit=20,
        ...
        "algorithm_recompute": {
            ...
            "candidate_counts": collect_dicts(
                ...
                SELECT algo_name || '@' || algo_version AS algo_key, status, count(*) AS asset_count
                FROM demo.robot.silver_asset_algo_latest
                WHERE algo_name = 'hand_tracking' AND algo_version = '1.2.0'
                GROUP BY algo_name, algo_version, status
                ORDER BY status
                """
            ),
