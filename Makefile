...
dev-down:
	cd deploy/local && docker compose --profile full --profile lakehouse down || true
...
all-reset-volumes:
	cd deploy/local && docker compose --profile full --profile lakehouse down -v || true
...
iceberg-mvp-host:
	cd deploy/local && docker compose --profile lakehouse exec -T \
		-e POSTGRES_URL=jdbc:postgresql://host.docker.internal:5432/cyber_databrew_dev \
...
# Curl smoke aligned with docs/review/api-guide.md. Override BASE, TOKEN, IAP_TOKEN as needed.
api-guide-smoke:
	bash scripts/api-guide-smoke.sh

# GKE in-cluster smoke (cyber-databrew-dev); bypasses IAP. Optional: KUBE_CONTEXT=...
api-guide-smoke-incluster:
	bash scripts/api-guide-smoke-incluster.sh

# Large-scale API-only scenario (tag → queries/run → delivery). Default BASE = hosted dev Cloud Run.
# Local: BASE=http://127.0.0.1:8080 ES_WAIT=0 make scenario-tags-bulk
scenario-tags-bulk:
