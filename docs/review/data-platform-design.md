...
| 某算法版本变更后哪些 asset 要重刷 | BigQuery 查 Iceberg `gold_recompute_candidates` |
| 某 tag 何时被算法追加 | `asset_events` (event_type=tag_upserted) → Iceberg 长期 |
| 上月所有 MCAP segment 质量分布 | BigQuery + Iceberg silver/gold |
...
| OLTP 全量替代分析查询 | PG 不背分析负载，分析查询走 BigQuery + Iceberg；想跑大宽表全量扫描不要打到 PG |
...
| 2.0 | + CDC/WAL + Elasticsearch + Iceberg + BigQuery | 启用 CDC/WAL 驱动同步，PG 变更推动 ES / Iceberg 派生；检索与分析分流，PG 只承担在线业务 |
...
| 湖仓 | 无 | 引入 BigLake-managed Iceberg + BigQuery，PG → Bronze → Silver → Gold |
...
| 查询层 | BigQuery | 查询 Iceberg，服务复杂分析与离线报表 |
...
sequenceDiagram
    ...
    UC->>BQ: POST /v1/statement<br>SELECT * FROM iceberg.gold.dataset_xxx<br>FOR VERSION AS OF :snapshot_id<br>LIMIT 100
    BQ->>Cat: 解析 catalog / namespace / table
    Cat-->>BQ: table metadata
    BQ->>Iceberg: 读 manifest + parquet
    Iceberg-->>BQ: rows
    BQ-->>UC: rows[]
...
- BigQuery 仅作"读"，不写入；MERGE 由 PyIceberg CronJob 异步做（详见 §5.6.2）
- 前端真要交互式探索时，可走专门的 BigQuery Console（如 SuperSet）；本接口面向程序化拉数
...
| 查 dataset 快照 | `GET /lakehouse/.../preview` | BigQuery → Iceberg time travel | < 5 s P99（首次查询） |
...
| 历史 / 分析 / 训练数据集 | **Apache Iceberg** + **BigQuery**（SQL 查询）+ **PyIceberg**（写入 / MERGE / compact） | 跨月 / 跨年大规模查询、数据集快照、训练复算 | 2.0 候选，未启动 |
...
> **状态更新（2026-05-14）**：本节描述的「Outbox Worker Bronze Sink → GCS staging → PyIceberg CronJob MERGE」两段式蓝图基于自建 Polaris/Lakekeeper catalog 的假设；**当前实现走的是 GCP 托管 BigLake REST Catalog + PyIceberg 直接 append**，路径短得多，不需要 staging、不需要 MERGE 脚本、不需要 Go 端 Bronze Sink。详见 [`docs/review/lakehouse-incremental-ingestion.md`](lakehouse-incremental-ingestion.md)。下面的两段式仍保留作为「外部 catalog 自建场景」的参考方案。
...
| 维度 | 传统数仓 | Lakehouse（Iceberg + BigQuery） |
| --- | --- | --- |
| 存储成本（每 TB / 月） | $20–25 | $4–8（GCS Standard）；冷分层后再砍一半 |
| 数据所有权 | 在 vendor 内部 | 在自己 GCS bucket |
| 引擎绑定 | 锁死 vendor 自有引擎 | 同一份数据 BigQuery / Spark / PyIceberg / DuckDB / Daft 都能读 |
...
| 引擎中立性 | **最强**（BigQuery / Spark / Flink / PyIceberg / Daft / DuckDB / Snowflake / BigQuery 一等支持） | 偏 Spark / Databricks | 偏 Spark |
...
| 多引擎读（BigQuery 给业务、PyIceberg 给训练 SDK、未来 Daft 给特征工程） | ✅ Iceberg 的强项 |
...
sequenceDiagram
    ...
    API->>BQ: SELECT ... FOR VERSION AS OF :snap LIMIT 100
    BQ->>Iceberg: 读 manifest + parquet
    BQ-->>API: rows[]
    API-->>TE: 200 OK
