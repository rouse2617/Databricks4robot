...
| F1.6 | 组件版本管理（迭代不影响已有 pipeline） | P1 | ✅ CYB-1284（需 migration，阻塞） |
| F1.7 | 组件搜索/筛选（?q=&source=） | P1 | ✅ CYB-1283 |
| F1.8 | 组件来源标注（system / custom / marketplace） | P2 | ✅ CYB-1286 |
...
| F2.8 | 删除节点/边 | P0 | ✅ 已实现 |
| F2.9 | 资产选择绑定（运行前选择要处理的 asset） | P1 | ✅ CYB-1285 |
| F2.10 | 节点参数模板默认值 | P1 | ✅ CYB-1281 |
| F2.11 | Pipeline 自动布局（DAG 层次化排列） | P2 | ✅ CYB-1270 |
| F2.12 | Pipeline 版本对比（diff） | P2 | ✅ CYB-1277 |
| F2.13 | 组件组/子图（嵌套 DAG） | P2 | ✅ CYB-1278 |
...
| F3.9 | 并行度控制（DAG 中并行执行的最大节点数） | P1 | ✅ CYB-1269 |
...
| F4.2 | 运行参数透传（asset path、metadata 传给容器） | P1 | ✅ CYB-1272 |
| F4.3 | 处理结果自动注册新 asset（容器回调 API） | P1 | ✅ CYB-1273 |
| F4.4 | pipeline 执行记录写入 asset_events | P2 | ✅ CYB-1274 |
| F4.5 | asset ↔ pipeline run 血缘查询 | P2 | ✅ CYB-1276 |
| F4.6 | 批量 asset → pipeline（一次 run 处理 N 个 asset） | P2 | ✅ 已有 backfill（CYB-1267）覆盖 |
| F4.7 | pipeline 产出自动关联输入 asset 血缘 | P2 | ✅ CYB-1275 |
...
→ 返回: 哪次 run、哪个 step、什么镜像产生的 ✅ CYB-1276
...
| F5.4 | Step 日志查看 | P1 | ✅ CYB-1268 (GET /workflows/:name/logs?nodeId=xxx) |
| F5.5 | 手动重试失败 step | P1 | ✅ CYB-1268 (POST /deployments/:id/retry) |
| F5.6 | 终止运行中的 workflow | P1 | ✅ CYB-1268 (POST /deployments/:id/stop) |
| F5.7 | 运行时间线 / Gantt 图 | P2 | ✅ CYB-1271 |
| F5.8 | 资源使用量展示（CPU/Mem 实际使用 vs request） | P2 | ✅ CYB-1279 |
...
| F7.8 | Template 从部署记录快速保存 | P2 | ✅ CYB-1280 |
...
| F5.4/F5.5/F5.6 Step 操作（日志/重试/终止） | handler ~100 行 + 前端 ~150 行 | Argo API | ✅ CYB-1268 |
...
| 功能 | 预计 |
|------|------|
| F4.2 运行参数透传（asset path → env） | ~80 行 | ✅ CYB-1272 |
| F4.3 容器产出注册回调 API | ~100 行 | ✅ CYB-1273 |
| F4.4 pipeline 事件记录 | ~20 行 | ✅ CYB-1274 |
| F4.7 产出血缘关联（asset_relations） | ~30 行 | ✅ CYB-1275 |
| F4.5 血缘查询接口 | ~50 行 | ✅ CYB-1276 |
| F4.6 批量处理 | ~20 行 | ✅ 已有 backfill（CYB-1267） |
| F2.9 部署前选择资产 | 前端 ~80 行 | ✅ CYB-1285 |
| F2.11 自动布局 | 前端 ~100 行 | ✅ CYB-1270 |
| F2.12 Pipeline diff | 前端 ~120 行 | ✅ CYB-1277 |
| F2.13 子图/嵌套 DAG | transpiler ~150 行 | ✅ CYB-1278 |
| F5.7 时间线 / Gantt 图 | ~80 行 | ✅ CYB-1271 |
| F5.8 资源使用量展示 | ~120 行 | ✅ CYB-1279 |
| F7.6/F7.7 Template 协作 | ~80 行 | ⏳ 需 migration（blocked） |
| F7.8 从部署记录保存 template | ~40 行 | ✅ CYB-1280 |
| F1.7 组件搜索/筛选 | 后端 ~30 行 | ✅ CYB-1283 |
| F1.8 system 组件种子数据 | 后端 ~30 行 | ✅ CYB-1286 |
| F3.9 并行度控制 | transpiler ~30 行 | ✅ CYB-1269 |
