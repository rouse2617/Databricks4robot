...
| 维度 | 当前态 | 下一里程碑 |
|------|--------|------------|
| 架构基线 | 1.0（PG + Backend 单进程） | 启用 2.0：CDC + ES + Iceberg |
| 投影表 | `asset_tags / asset_algo_latest` 已建已用，cf_* 列只读保留 | 字段消费收口 + lifecycle_state 主消费切换 |
| 事件流 | `asset_events` 在线写入；CDC 消费投递 ES/Bronze | 预发跑满 CDC 链路验收 / 24h SLO 验收 |
| Tag 子域 | 独立 tag CRUD + history API 已上线；详情页 / 批量操作已切专用 tag 接口 | 下一步：tag history UI / `cf_*` 兼容层清理 |
| 检索 | `/api/v1/search/assets` ES path + **CDC→ES 索引写入 / admin reindex**；PG fallback 仍在 | PG↔ES 一致率闸口、生产索引初始化 runbook |
...
| 2 | **P0-4** | CDC 消费链路 MVP（Bronze + ES current-state），满足 `cdc-rollout-plan.md` 阶段验收。 |
...
- 「**1.0 → 2.0**」正式闸口仍见 `data-platform-design.md` §9.1.1；**MVP v1 交付 ≈ 该表 ES + Pure CDC + 投影 + lifecycle 相关行通过**，PgBouncer 若未做须在评审纪要里记 **已知技术债 + 补救时间点**。

## 0.6 Phase 1.5（Eval Metrics MVP）

> 新增子阶段，位于 1.0 收口与 2.0 全量启用之间。专项设计见 `eval-metrics-design.md`。

| 顺序 | 任务 | DoD |
|------|------|-----|
| 1 | 增加 `metric_registry`（先 YAML） | key/type/unit/queryable 可维护，未注册 key 监控可见 |
| 2 | 增加 `asset_eval_results` / `asset_metrics` | DDL 合并，主键含 `eval_version`，索引可支撑范围查询 |
| 3 | 上线 K1/K2/K3 核心 API | 单事务写入原始结果 + 投影 + 事件 |
| 4 | 上线 eval/metric 事件 | `eval_result_reported` / `metric_upserted` 可观测、可重放 |
| 5 | 前端详情页展示 metrics latest | 资产详情可见关键质量指标 |
...
|----|------|----------------|------|-------|------|----------|
| **P0-1** | **Schema 字段提升收口**：核对 `assets / mcap_files` 上 `asset_type / lifecycle_state / end_timestamp_ns / duration_ms / owner / retention_tier / expire_at` 已在 migration / repo / OpenAPI / filter 白名单一致；只补缺口，不重做已上线字段 | 收口 PR 合入；仍缺的字段/索引补齐；`schemas/pg-phase0.sql` 与 `schema-reference.md` 同步；明确留下哪些 legacy 字段仍处兼容期 | M | backend | done | P0-1 收口：AssetRepo.Get 补读 parent_asset_id/root_asset_id/split_*/metadata/files/tenant_id/project_id（35 列）；ListByMcapFile/ListWithFilters 补读 parent/root/tenant/project（27 列）；SoftDelete 补 lifecycle_state='archived'；Filter 白名单加 segment_locator/parent_asset_id/root_asset_id/asset_level/storage_uri/tenant_id/project_id；McapFile model 补 provenance 全字段 + retention_tier/expire_at/metadata；McapFileRepo Get/Set/List 读写全部 DDL 列（31 列）；file_duration_ms 不再硬编码 0；migration 008 补 asset_events 的 retry_count/last_error/aggregate_type 三列；pg-phase0.sql 同步；Model 注释清理 Bigtable 引用 |
...
| **P0-4** | **CDC 消费链路 MVP**（Bronze + ES current-state） | 完整覆盖 `cdc-rollout-plan.md` 阶段验收；ES 索引投递端到端 P99 ≤ 60s；持续 24h 投递成功率 > 99.9% | L | backend | in-progress | 代码：`internal/cdc`（含 `bronze_sink`）；**SLO 需预发实测** |
| **P0-5** | **`/admin/search/reindex` 全量重建 API**（同步交付） | 给定 dry_run / rebuild_index 参数；扫 `assets` 全表重建 ES doc；不改 CDC 消费位点（offset）；当前先复用 `X-Grace-Token`，后续再评估是否切回独立 admin 鉴权 | S | backend | done | `POST /api/v1/admin/search/reindex` |
...
| ID | 任务 | DoD | 估时 | 状态 | 落地证据 |
|----|------|-----|------|------|----------|
| **P0-T-1** | **CDC 集成测试**：testcontainers-go 起 PG + Kafka/Connect + ES 容器，覆盖 Bronze 与 ES current-state 消费 | CDC 端到端用例全部通过；`make test-integration` 一键跑 | L | blocked-by-P0-4 | — |
...
|----|------|-----|------|-------|------|----------|
| **P1-1** | **ES `assets` 索引正式启用** | mapping = `init-index.sh`；接入 CDC 后 ES 文档数与 PG `assets` 一致率 >99.9% | M | backend | in-progress | mapping 脚本就绪；写入路径已接 CDC；**闸口对账待跑** |
| **P1-2** | **CDC 监控 + 告警**：Prometheus metrics（consumer lag / decode errors / rebuild latency）+ Grafana dashboard | dashboard 上线；端到端延迟 / 失败率 / consumer lag 三个核心 panel；接 PagerDuty 告警阈值 | M | backend + 运维 | in-progress | **MVP 轻量**：`GET /metrics`（`cdc_*`）+ `backend/README.md` 阈值建议；Grafana / PagerDuty 下一迭代 |
...
| **P1-8** | **CDC 消费器独立进程化**（先内嵌 90 天稳定再切） | 独立 K8s Deployment；切换无丢事件 | M | backend + 运维 | blocked-by-P0-4 | — |
| **P1-9** | **`asset_events` 收窄为“资产域事件流”**：冻结保留事件清单，停止把 delivery / training / eval / metrics 默认接入 `asset_events`；代码层先收窄 `asset_updated` 的发射条件 | 评审口径以 `asset-events-thinning-plan.md` 为准；`asset_updated` 不再覆盖纯 lifecycle/tag 变更；`data-platform-design.md` / `schema-reference.md` / `use-cases.md` / `algo-lifecycle-and-data-model.md` / `eval-metrics-design.md` / `api-guide.md` 同步收窄 | M | backend + architecture | todo | `docs/review/asset-events-thinning-plan.md` |
| **P1-10** | **`actions` 表上线**（业务模型 `mcap → seg → action` 第三层）：①  schema：建 `actions` 表 + `assets.summary_text` 列；② repo + usecase + handler（`/api/v1/assets/:id/actions` + 平台级 `/api/v1/actions`）；③ events：新增 `action_upserted` / `action_deleted` schema + registry；④ CDC ES 反向投影：interval 事件 → 重读该 seg 全部 actions → upsert seg ES doc 的 `actions[]` nested；⑤ SDK + 前端时间轴展示（候选挂在 `AssetDetailPage`）；⑥ vitest / go test / E2E 覆盖 | DDL 落地；handler 单测覆盖创建 / 修改 / 软删 / 时间戳点查 / 区间查 / 平台反查；ES seg 文档可见 `actions[]`；SDK + 前端可读写 | L | backend + frontend + sdk + data | todo | 设计：`data-platform-design.md §5.1` 时间区间小节 + §5.2.15 actions + `schema-reference.md#actions` + `api-guide.md §2.7` |
| **P1-11** | **`mcap_files.valid_ranges` 投影**（文件有效时间段集合）：① schema：`ALTER TABLE mcap_files ADD COLUMN valid_ranges int8multirange NOT NULL DEFAULT int8multirange()` + GiST 索引；② 投影回写：`asset_created` / `asset_lifecycle_changed` / `asset_deleted` 同事务（或紧随 CDC 投影）→ 聚合该 mcap 下 ready/delivered seg 的 `range_agg(int8range)` 写回；③ API：`GET /api/v1/mcap/{id}` 响应包含 `valid_ranges`，写入路径**禁止**接受该字段；④ 检索：暴露 `?at=&overlaps=` 参数走原生 multirange 算子；⑤ 测试：seg lifecycle 变更后 mcap.valid_ranges 一致；点查 / 区间查 / overlap 验证 | DDL 落地；投影回写在 seg 事件单测里覆盖；GET 端点显式返回 valid_ranges；overlap 检索 E2E 通过 | M | backend + data | todo | 设计：`data-platform-design.md §5.2.3` 写入契约 + `schema-reference.md#mcap_files` |
...
| **P1-T-2** | **CDC 性能 / 吞吐压测**：50 events/s × 60s 持续 → 端到端 P99 ≤ 60s（G1） | 基线报告归档；规模上扬到 200 events/s 时 consumer 行为可观测 | S | blocked-by-P0-T-1 | — |
...
| **P2-3** | ES 写 `tagged_at` 字段，支持「最近一天打的 tag」过滤 | 数据治理 / 抽样需求；mapping + CDC projector 都要改 | S | todo |
...
| **P2-T-2** | CDC chaos 测试（`testcontainers` 注入 Kafka/PG/ES 故障）| CDC MVP 跑稳 60 天后再加 | M | parked |
