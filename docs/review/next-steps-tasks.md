...
| **P0-4** | **CDC 消费链路 MVP**（Bronze + ES current-state） | 完整覆盖 `cdc-rollout-plan.md` 阶段验收；ES 索引投递端到端 P99 ≤ 60s；持续 24h 投递成功率 > 99.9% | L | backend | in-progress | 代码：`internal/cdc` + `internal/outbox/bronze_sink`；**SLO 需预发实测** |
