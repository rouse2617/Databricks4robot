...
| 事件 / 审计 / CDC | `asset_events`、`outbox_sink_cursors`（兼容）、`outbox_dlq`（兼容） |
...
| updated_at             | TIMESTAMPTZ | 是   | 更新时间                                | ES / Iceberg     |
| version                | BIGINT      | 是   | 乐观锁版本                               | Iceberg          |
| status                 | VARCHAR(32) | 是   | Phase 0 遗留审核态（PG 默认 `approved`）；与 `lifecycle_state` 并存，**权威流转以 `lifecycle_state` 为准** | ES / Iceberg（若同步） |
| segment_locator        | CHAR(40)    | 否   | 片段定位（遗留；与 ingest/MCAP 锚定相关） | 按需            |
| cf_meta / cf_algo / cf_tag / cf_files | JSONB | 是（默认可 `{}`） | 列族兼容列；与 `asset_tags` / `asset_algo_latest` 双写过渡期保留 | Iceberg |
...
- **主键**：event_id
- **单调序号**：event_seq BIGSERIAL UNIQUE，所有 CDC / replay 消费者必须按 event_seq 严格递增消费，不能用 occurred_at 排序
...
- `publish_state` / `retry_count` / `last_error` / `published_at` 属于历史兼容字段；当前生产运行时消费位点与重试策略以 CDC consumer 机制为准。`outbox_dlq` 作为历史兼容死信承载保留。
