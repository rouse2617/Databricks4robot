...
> 当前生产与上线口径统一采用 Pure CDC（WAL + Debezium/Kafka + CDC consumers），请优先参考 [`pure-cdc-go-live-runbook.md`](./pure-cdc-go-live-runbook.md) 与 [`cdc-rollout-plan.md`](./cdc-rollout-plan.md)。
>
...
| `asset_id`       | TEXT                    | dedup 与投影主键（与 `assets.asset_id` 同型）                                                                                                                      |
...
4. **不改 CDC 消费位点（offset）**，也不改历史兼容字段 `publish_state`——这是"基于源表的全量重建"，与事件流并行
