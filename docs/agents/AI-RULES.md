...
5. **Branch** — Use `fix/CYB-{id}-*`, `feat/CYB-{id}-*`, or `hotfix/CYB-{id}-*` as appropriate (`DAT-*` accepted by CI for legacy). Create the branch when starting OpenSpec or immediately after the OpenSpec checkpoint passes.
