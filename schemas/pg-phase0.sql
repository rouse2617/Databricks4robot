...
--                 / asset_tags / asset_algo_latest / asset_events / search_reindex_jobs / idempotency_keys
...
CREATE TABLE IF NOT EXISTS mcap_files (
    mcap_file_id        TEXT         PRIMARY KEY CHECK (mcap_file_id ~ '^[0-9A-Za-z]{8}$'),
    ...
    -- 异步 MCAP summary 索引状态（docs/review/mcap-index-metadata-v1.md）
    summary_index_state    TEXT        NOT NULL DEFAULT 'pending'
        CHECK (summary_index_state IN ('pending','running','done','failed')),
    summary_index_version  TEXT,
    summary_indexed_at     TIMESTAMPTZ,
    summary_index_attempts INT         NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_mcap_files_summary_index_pending
    ...
    WHERE is_deleted = FALSE AND summary_index_state <> 'done';
...
CREATE TABLE IF NOT EXISTS assets (
    ...
    mcap_file_id            TEXT         NOT NULL REFERENCES mcap_files(mcap_file_id)
                                CHECK (mcap_file_id ~ '^[0-9A-Za-z]{8}$'),
    ...
    algo_inputs_uris        JSONB        NOT NULL DEFAULT '{}'::jsonb,
    annot_inputs_uris       JSONB        NOT NULL DEFAULT '{}'::jsonb,
...
CREATE INDEX IF NOT EXISTS idx_assets_algo_inputs_uris_gin
    ON assets USING GIN (algo_inputs_uris);
CREATE INDEX IF NOT EXISTS idx_assets_annot_inputs_uris_gin
    ON assets USING GIN (annot_inputs_uris);
...
CREATE TABLE IF NOT EXISTS asset_relations (
    parent_asset_id         TEXT         NOT NULL REFERENCES assets(asset_id),
    child_asset_id          TEXT         NOT NULL REFERENCES assets(asset_id),
...
-- =====================================================================
-- 3.1 actions —— seg 内时间分段标注 (mcap → seg → action 第三层)
--      docs/review/data-platform-design.md §5.2.15 / docs/review/schema-reference.md#actions
-- 约定：
...
--   · start_ns / end_ns 与 assets.start_timestamp_ns / end_timestamp_ns 同口径 (相对 mcap 起点 ns)
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS asset_events (
    ...
    mcap_file_id            TEXT         REFERENCES mcap_files(mcap_file_id)
                                CHECK (mcap_file_id IS NULL OR mcap_file_id ~ '^[0-9A-Za-z]{8}$'),
    tenant_id               TEXT,
    project_id              TEXT,
    ...
    created_at              TIMESTAMPTZ  NOT NULL DEFAULT now(),
    publish_state           TEXT         NOT NULL DEFAULT 'pending',  -- pending / processing / published / failed / dlq
...
-- =====================================================================
-- 6.1 search_reindex_jobs —— 手动全量重建 ES 的异步任务状态
-- =====================================================================
...
COMMENT ON TABLE search_reindex_jobs IS
    '异步重建 ES 索引任务：支持停止、断点续开、前端进度查询。';

-- =====================================================================
-- 6.2 es_sync_checkpoint —— 订阅方 PG→Pub/Sub→ES 每分片水位
-- =====================================================================
...
COMMENT ON TABLE  es_sync_checkpoint              IS 'ES subscriber per-shard 已应用 event_seq 水位（暴露为 /search/sync-progress 的 consumer_lag）。';
COMMENT ON COLUMN es_sync_checkpoint.shard_id     IS 'FNV(routing_key) % OUTBOX_ES_CHECKPOINT_SHARDS；部署期内稳定。';
COMMENT ON COLUMN es_sync_checkpoint.applied_seq  IS 'GREATEST(applied_seq, batch_max_seq)；仅在 ES bulk 成功后推进。';

-- =====================================================================
-- 6.3 lakehouse_bronze_checkpoint —— PG→Iceberg Bronze 水位线
-- =====================================================================
...
COMMENT ON TABLE asset_eval_results IS '资产评估运行原始事实。docs/review/eval-metrics-design.md §4.1。';
...
COMMENT ON TABLE asset_metrics IS '可检索指标投影，不扫 JSONB。docs/review/eval-metrics-design.md §4.2。';

-- triggers for updated_at
DO $$
BEGIN
    DROP TRIGGER IF EXISTS trg_asset_eval_results_updated_at ON asset_eval_results;
    CREATE TRIGGER trg_asset_eval_results_updated_at
        BEFORE UPDATE ON asset_eval_results
        FOR EACH ROW EXECUTE FUNCTION set_updated_at();

    DROP TRIGGER IF EXISTS trg_asset_metrics_updated_at ON asset_metrics;
