...
/** Cyber login page URL — override via window.__cyber_login_url or settings. */
function loginUrl(): string {
    const custom = (window as any).__cyber_login_url;
    return custom || '/login';
}
...
function auth(req: SuperAgentRequest) {
    ...
    if (token) {
        req.set('X-Databrew-Token', token);
    }
...
function handle(err: any) {
    ...
    if (err.status === 401 && !document.location.href.includes('login')) {
        ...
        document.location.href = uiUrlWithParams(loginUrl(), params);
    }
}
