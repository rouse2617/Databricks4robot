		...
		>
			...
			>
				<div>
					...
					>
						{hasTrend && trend === "up" && (
							<ArrowUpOutlined style={{ color: "#16A34A", marginRight: 4 }} />
						)}
						{hasTrend && trend === "down" && (
							<ArrowDownOutlined style={{ color: "#DC2626", marginRight: 4 }} />
						)}
						<Text
							type="secondary"
				...
				>
					<span style={{ color: color ?? "#2563EB", fontSize: 18 }}>
						{icon}
					</span>
				</div>
			</div>
...
function deriveFromSampling(
	...
	const successRate =
		algoTotal > 0 ? ((algoCounts.ok / algoTotal) * 100).toFixed(1) : "—";
	const deliveryTotal = assets.reduce(
		(sum, a) => sum + (a.delivery_count ?? 0),
		0,
	);
	const assetsWithDeliveries = assets.filter(
		(a) => (a.delivery_count ?? 0) > 0,
	).length;
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				...
				if (!cancelled) {
					setDerived(
						deriveFromSampling(data.items ?? [], data.total ?? 0, mcapTotal),
					);
				}
			} catch {
	...
	const failedColumns: ColumnsType<Asset> = [
		{
			...
			render: (id: string) => (
				<button
					...
					onClick={() => navigateToAssetDetail(navigate, id)}
				>
					{id.slice(0, 8)}…
				</button>
			),
		},
		{
			...
			render: (_: unknown, record: Asset) =>
				extractAlgoStatuses(record.algo_results)
					...
					.map((f) => (
						<Tag color="error" key={f.key} style={{ fontSize: 11 }}>
							{f.key.split("@")[0]}
						</Tag>
					)),
		},
	...
	if (!derived) {
		return (
			<div style={{ maxWidth: 1400 }}>
				<Title level={4} style={{ margin: "0 0 16px 0", fontWeight: 600 }}>
					概览
				</Title>
				<Alert
					type="error"
											...
											<List.Item style={{ padding: "8px 0" }}>
												<Space wrap>
													<button
														...
														className="font-mono text-xs cursor-pointer link-like-button"
														onClick={() =>
															navigateToAssetDetail(navigate, a.asset_id)
														}
													>
					...
					columns={[
						...
						},
						{
							...
							render: (_: unknown, r: Asset) => (
								<Tag color={getAssetStateColor(r)}>
									{getLifecycleState(r) || "—"}
								</Tag>
							),
						},
								...
								return (
									<Space size={2}>
										{statuses.slice(0, 3).map((s) => (
											<Tag
												key={s.key}
												color={algoStatusColor[s.status]}
