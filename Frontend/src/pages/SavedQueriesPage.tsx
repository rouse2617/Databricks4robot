...
function savedQueryToEditorState(item?: SavedQuery): EditorState {
	return {
		...
		queryText: JSON.stringify(
			item?.query_ir_json ?? defaultQueryRequest(),
			null,
			2,
		),
		validation: null,
...
export default function SavedQueriesPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			void msg.error(
				err instanceof Error ? err.message : "加载 saved queries 失败",
			);
		} finally {
			setLoading(false);
		}
			...
			},
			{
				...
				render: (_value, row) => (
					<Space>
						<Button
							size="small"
							onClick={() => setEditor(savedQueryToEditorState(row))}
						>
							编辑
						</Button>
						<Button
							...
							onClick={() => {
								sessionStorage.setItem(
									"assets_open_saved_query_id",
									`remote:${row.saved_query_id}`,
								);
								navigate("/assets");
							}}
						>
	...
	return (
		<div>
			...
			>
				...
				<Button
					type="primary"
					onClick={() => setEditor(savedQueryToEditorState())}
				>
					新建 Query
				</Button>
			</div>
			...
			<Modal
				open={editor.open}
				title={
					editor.mode === "create" ? "新建 Saved Query" : "编辑 Saved Query"
				}
