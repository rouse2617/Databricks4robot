...
type AssetLike = Pick<
  Asset,
  "duration_ms" | "duration_sec" | "lifecycle_state" | "status" | "asset_type" | "type"
>;

export function getDurationMs(asset: AssetLike): number | null {
  if (typeof asset.duration_ms === "number" && !Number.isNaN(asset.duration_ms)) {
    return asset.duration_ms;
  }
  if (typeof asset.duration_sec === "number" && !Number.isNaN(asset.duration_sec)) {
