...
function parseFilterParam(raw: string): FilterChip | null {
	...
	const rawField = raw.slice(0, firstColon);
	const op = raw.slice(firstColon + 1, secondColon);
	const rawValue = raw.slice(secondColon + 1);

	if (!rawField || !op) return null;
	const field =
		rawField === "tags_flat.priority"
			? "tag.priority"
			: rawField === "tags_flat.quality"
				? "tag.quality"
				: rawField;
...
export function parsePreviewAssetIdFromUrl(sp: URLSearchParams): string | null {
	const parsed = parsePreviewURLState(sp);
	if (parsed.preview) return parsed.preview;
	const viaDs = parsed.dsParams?.asset_id ?? parsed.dsParams?.assetId;
	return viaDs ?? null;
}

export function parsePreviewTopicFromUrl(sp: URLSearchParams): string | null {
	return parsePreviewURLState(sp).topic ?? null;
}

export function parsePreviewSourceIdFromUrl(
	sp: URLSearchParams,
): string | null {
	return parsePreviewURLState(sp).source ?? null;
}
 ...
 * When `previewAssetId` is a non-empty string, adds `preview=<asset_id>` for shareable workbench state.
 */
export function serializeQueryStateToUrl(
	state: QueryState,
	previewAssetId?: string | null,
	previewSourceId?: string | null,
	previewTopic?: string | null,
	previewTime?: number | null,
	dsState?: { ds?: string | null; dsParams?: Record<string, string> | null },
): URLSearchParams {
	...
	for (const chip of state.activeFilters) {
		if (state.searchMode === "keyword" && chip.field === "_fulltext") {
			continue;
		}
	...
	sp = updatePreviewURLState(sp, {
		preview: previewAssetId ?? undefined,
		source: previewSourceId ?? undefined,
		topic: previewTopic ?? undefined,
		time: previewTime ?? undefined,
		ds: dsState?.ds ?? undefined,
		dsParams: dsState?.dsParams ?? undefined,
	});
 ...
 */
export function parseQueryStateFromUrl(
	sp: URLSearchParams,
): Partial<QueryState> {
	const result: Partial<QueryState> = {};

	const mode = sp.get("mode");
	if (mode && isSearchMode(mode)) {
	...
	if (result.searchMode === "keyword" && result.activeFilters) {
		result.activeFilters = result.activeFilters.filter(
