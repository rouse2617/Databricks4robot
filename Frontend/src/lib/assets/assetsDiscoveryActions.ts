...
export type AssetsDiscoveryAction =
  ...
  // ── Query Commit (11) ──
  | { type: "COMMIT_QUERY_TEXT"; payload: { text: string; tokens: FilterChip[] } }
  ...
  | { type: "FACET_RANGE_DRAFT"; payload: { field: string; min?: number; max?: number } }
  | { type: "FACET_RANGE_APPLY"; payload: { field: string; min?: number; max?: number } }
  | { type: "FACET_DATE_DRAFT"; payload: { field: string; start?: string; end?: string } }
  | { type: "FACET_DATE_APPLY"; payload: { field: string; start?: string; end?: string } }
  ...
  | { type: "RECEIVE_PREVIEW_SUCCESS"; payload: { asset: Asset; manifest: PreviewManifest } }
  ...
  | { type: "LAYOUT_RESPONSIVE"; payload: { isTablet: boolean; isMobile: boolean } }
