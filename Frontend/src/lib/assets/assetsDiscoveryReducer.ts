...
export function assetsDiscoveryReducer(
	...
	switch (action.type) {
		...
		case "URL_HYDRATE": {
			...
			const psid = action.payload.previewSourceId;
			const ptopic = action.payload.previewTopic;
			const ptime = action.payload.previewTimeSec ?? null;
			...
			return {
				...
				previewState: {
					...
					activeTopic: ptopic,
					activeTimeSec: ptime,
					ds: action.payload.ds ?? null,
					dsParams: action.payload.dsParams ?? null,
					fetchStatus: pid ? "loading" : "idle",
					summary: pid ? state.previewState.summary : null,
					manifest: pid ? state.previewState.manifest : null,
				},
		...
		case "FACET_RANGE_APPLY": {
			...
			if (hasMin && hasMax) {
				...
				newFilters.push(createFilterChip(field, ">=", String(min), "facet"));
			} else if (hasMax) {
				newFilters.push(createFilterChip(field, "<=", String(max), "facet"));
			}
			return withQueryReset(state, {
				...state.queryState,
				activeFilters: newFilters,
		...
		case "FACET_DATE_APPLY": {
			...
			if (hasStart && hasEnd) {
				...
				newFilters.push(
					createFilterChip(field, ">=", start as string, "facet"),
				);
			} else if (hasEnd) {
				newFilters.push(createFilterChip(field, "<=", end as string, "facet"));
			}
			return withQueryReset(state, {
				...state.queryState,
				activeFilters: newFilters,
		...
		case "RESULTS_SUCCESS":
			return {
				...
				resultsState: {
					...
					isStale: false,
					aggregations:
						action.payload.aggregations ?? state.resultsState.aggregations,
		...
		case "SET_ACTIVE_PREVIEW_ASSET":
			return {
				...
				previewState: {
					...
					activeSourceId: action.payload.assetId
						? state.previewState.activeSourceId
						: null,
					activeTopic: action.payload.assetId
						? state.previewState.activeTopic
						: null,
					activeTimeSec: action.payload.assetId
						? state.previewState.activeTimeSec
						: null,
					ds: action.payload.assetId ? state.previewState.ds : null,
					dsParams: action.payload.assetId ? state.previewState.dsParams : null,
					fetchStatus: action.payload.assetId ? "loading" : "idle",
					summary: action.payload.assetId ? state.previewState.summary : null,
					manifest: action.payload.assetId ? state.previewState.manifest : null,
					availability: action.payload.assetId
						? state.previewState.availability
						: "missing",
		...
		case "SET_PREVIEW_SOURCE":
			return {
				...
				previewState: {
					...
					fetchStatus: state.previewState.activeAssetId ? "loading" : "idle",
					retryNonce: state.previewState.retryNonce + 1,
				},
			};
		...
		case "SET_PREVIEW_TIME":
			return {
				...
				previewState: {
					...state.previewState,
					activeTimeSec: action.payload.timeSec,
				},
			};
		...
		case "RECEIVE_PREVIEW_SUCCESS":
			return {
				...
				previewState: {
					...
					ds: action.payload.manifest.ds ?? state.previewState.ds,
					dsParams:
						action.payload.manifest.dsParams ?? state.previewState.dsParams,
					activeSourceId:
						action.payload.manifest.activeSourceId ??
						state.previewState.activeSourceId,
		...
		case "PREVIEW_RETRY":
			return {
				...
				previewState: {
					...state.previewState,
					fetchStatus: state.previewState.activeAssetId ? "loading" : "idle",
					availability: state.previewState.activeAssetId
						? state.previewState.availability
						: "missing",
					retryNonce: state.previewState.retryNonce + 1,
				},
		...
		case "PREVIEW_COLLAPSE_TOGGLE":
			if (!state.previewState.collapsed) {
				// Collapsing → also clear URL preview params so sharing/session state is clean.
				return {
					...state,
					previewState: {
						...state.previewState,
		...
		case "PREVIEW_CLEAR":
			return {
				...
				previewState: {
					...
					retryNonce: state.previewState.retryNonce,
