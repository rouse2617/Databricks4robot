...
import { type QueryRequest, queryApi } from "../../api/query";
		...
		const load = async () => {
			try {
				...
				const remoteViews = (remote.items ?? []).reduce<SavedView[]>(
					(acc, item) => {
						const queryState = queryRequestToPartialQueryState(
							item.query_ir_json,
						);
						if (!queryState) {
							return acc;
				...
				const pendingOpenId = sessionStorage.getItem(
		...
		async (name: string) => {
			{
				...
				dispatch({
					...
					payload: {
						views: [
							...views.filter(
								(v) => v.id !== `remote:${remote.saved_query_id}`,
							),
							{
				...
				dispatch({
					...
					payload: { viewId: `remote:${remote.saved_query_id}` },
				});
