...
function TimestampInput({
	...
	const submit = () => {
		...
		if (Number.isFinite(sec) && sec >= 0) onSeek(Math.min(sec, durationSec));
	};
	if (editing) {
		return (
			<Input
			...
			>
				...
				<Tooltip title="后一帧">
					<Button
						...
						size="small"
						onClick={() => onSeek(Math.min(durationSec, currentTime + 1 / 30))}
					/>
				</Tooltip>
				<Select
					value={playbackRate}
				...
				<div style={{ flex: 1, minWidth: 0 }}>
					<RangeSlider
						...
						range={
							range ? { startSec: range.startSec, endSec: range.endSec } : null
						}
						onSeek={onSeek}
				...
				<Tooltip title={coverMode ? "完整画面" : "裁剪满屏"}>
					<Button
						...
						icon={
							coverMode ? (
								<ColumnWidthOutlined style={{ color: "var(--gray-500)" }} />
							) : (
