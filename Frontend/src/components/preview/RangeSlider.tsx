...
export default function RangeSlider({
	...
	const dragRef = useRef<{
		...
		startX: number;
		startSec: number;
		startRange?: { startSec: number; endSec: number };
	}>({ target: null, startX: 0, startSec: 0 });
	...
	const onMouseDown = useCallback(
		(e: React.MouseEvent, target: DragTarget) => {
			...
			dragRef.current = {
				...
				startX: e.clientX,
				startSec: sec,
				startRange: range ? { ...range } : undefined,
			};
		...
		<div
			...
			onMouseMove={(e) => {
				if (!dragTarget) {
					...
					setHoverX(
						e.clientX - (barRef.current?.getBoundingClientRect().left || 0),
					);
				}
			}}
		...
		>
			...
			<div
				style={{
					...
					borderRadius: 2,
					transition: dragTarget ? "none" : "background 0.15s",
				}}
			/>
			...
			{hasRange && (
				<div
					style={{
						position: "absolute",
						left: `${rangeStartFrac}%`,
						width: `${rangeEndFrac - rangeStartFrac}%`,
						top: "50%",
						height: 4,
						marginTop: -2,
			...
			{hasRange && (
				<div
					...
					style={{
						...
						background:
							dragTarget === "rangeEnd" ? "var(--color-accent)" : "#fff",
						borderRadius: "50%",
						cursor: "ew-resize",
						zIndex: 2,
						boxSizing: "border-box",
