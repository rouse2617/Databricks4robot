...
function deriveDurationMs(raw: Record<string, unknown>): number {
	...
	if (
		...
		return (
			(stats.window_effective_end_ns - stats.window_effective_start_ns) / 1e6
		);
	}
	return 0;
}
...
export default function PreviewPage() {
	...
	const handleLoad = useCallback(async (assetId: string) => {
		...
		try {
			...
			setActiveTopics(
				channels
					.filter((c) => !c.topic.toLowerCase().includes("side"))
					.map((c) => c.topic),
			);
			setCurrentTime(0);
			setPlaying(false);
			...
			setPreviewRange({
				startSec: 0,
				endSec: Math.min(120, Math.max(durationSec, 10)),
			});
	...
	const getVideoSrc = useCallback(
		(ch: PreviewChannel): string => {
			const src = loaded?.sources.find(
				(s) => s.topic === ch.topic || s.id === ch.topic,
			);
			if (!src) return "";
	...
	const handleVideoTimeUpdate = useCallback((t: number) => {
		...
		if (st) {
			...
			if (performance.now() < st.deadline && Math.abs(t - st.time) > 0.5)
				return;
	...
	const effectiveStartNs =
		win?.start_timestamp_ns || statsWin?.window_effective_start_ns || 0;
		...
		<div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
			...
			>
				...
				<LayoutSerializer
					activeTopics={
						loaded?.channels
							.filter((c) => activeTopics.includes(c.topic))
							.map((c) => c.topic) || activeTopics
					}
					coverMode={coverMode}
					...
					timeline={
						<UnifiedTimeline
							...
							range={
								previewRange
									? {
											startSec: previewRange.startSec,
											endSec: previewRange.endSec,
