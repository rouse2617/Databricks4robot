...
function createFrustum(
	...
	const ntl = nearCenter
		.clone()
		.add(camUp.clone().multiplyScalar(nearH))
		.add(right.clone().multiplyScalar(-nearW));
	const ntr = nearCenter
		.clone()
		.add(camUp.clone().multiplyScalar(nearH))
		.add(right.clone().multiplyScalar(nearW));
	const nbl = nearCenter
		.clone()
		.add(camUp.clone().multiplyScalar(-nearH))
		.add(right.clone().multiplyScalar(-nearW));
	const nbr = nearCenter
		.clone()
		.add(camUp.clone().multiplyScalar(-nearH))
		.add(right.clone().multiplyScalar(nearW));
	...
	const ftl = farCenter
		.clone()
		.add(camUp.clone().multiplyScalar(farH))
		.add(right.clone().multiplyScalar(-farW));
	const ftr = farCenter
		.clone()
		.add(camUp.clone().multiplyScalar(farH))
		.add(right.clone().multiplyScalar(farW));
	...
	[ntl, ntr, nbl, nbr].forEach((n, i) => {
		...
		group.add(
			new THREE.Line(new THREE.BufferGeometry().setFromPoints([n, f]), lineMat),
		);
	});
...
export default function Viewport3D({
	...
	useEffect(() => {
		...
		cameras.forEach((cam) => {
			...
			scene.add(frustum);
			const labelPos = new THREE.Vector3(...cam.position).add(
