...
function JsonNode({
	...
	if (value === null) {
		return (
			<div style={{ paddingLeft: indent, color: "var(--gray-400)" }}>
				{label}: <span style={{ color: "#ff6b6b" }}>null</span>
			</div>
		);
	}
	...
	if (typeof value === "boolean") {
		return (
			<div style={{ paddingLeft: indent }}>
				{label}: <span style={{ color: "#ffd43b" }}>{String(value)}</span>
			</div>
		);
	}
	...
	if (typeof value === "number") {
		...
		return (
			<div style={{ paddingLeft: indent }}>
				{label}: <span style={{ color: "#51cf66" }}>{numStr}</span>
			</div>
		);
	}
	...
	if (Array.isArray(value)) {
		return (
			<div>
				...
				>
					{expanded ? (
						<CaretDownOutlined style={{ fontSize: 10, marginRight: 4 }} />
					) : (
						<CaretRightOutlined style={{ fontSize: 10, marginRight: 4 }} />
					)}
						...
						<JsonNode
							key={
								typeof item === "object" && item !== null
									? JSON.stringify(item).slice(0, 32) || `arr-${i}`
									: `${typeof item}-${String(item).slice(0, 32)}`
							}
							label={`[${i}]`}
							value={item}
							depth={depth + 1}
		...
		return (
			<div>
				...
				>
					...
					<span style={{ color: "var(--gray-400)" }}>{label}</span>
					<span
						style={{ color: "var(--gray-500)", marginLeft: 4 }}
					>{`{${entries.length}}`}</span>
				</button>
				{expanded &&
					entries.map(([k, v]) => (
