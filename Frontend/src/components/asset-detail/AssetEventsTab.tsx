...
function summarizeEvent(event: AssetEvent): string {
  ...
  switch (event.event_type) {
    ...
    case "asset_lifecycle_changed": {
      const prev = payloadString(payload.prev_lifecycle_state) || payloadString(payload.prev_status) || "—";
      const next = payloadString(payload.new_lifecycle_state) || payloadString(payload.new_status) || "—";
      return `${prev} → ${next}`;
    }
    case "tag_upserted": {
    ...
    default:
      return payloadString(payload.tag_key) || payloadString(payload.algo_key) || "事件详情见 payload";
  }
}
    ...
    <Card size="small" title="全部事件">
      <Timeline
        items={events.map((event) => ({
          color:
            event.event_type.includes("failed") ? "red" :
              event.event_type.includes("deleted") ? "orange" :
                event.event_type.includes("created") || event.event_type.includes("upserted") ? "green" :
                  "blue",
          children: (
            <div>
              ...
              <div>
                <Text type="secondary" className="text-xs">
                  seq={event.event_seq} · {dayjs(event.created_at).format("MM-DD HH:mm:ss")}
                </Text>
              </div>
            </div>
          ),
