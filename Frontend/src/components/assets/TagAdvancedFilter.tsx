...
import { Alert, Button, Select, Input, Space, Card, Typography } from "antd";
...
export default function TagAdvancedFilter({ onApply, onClose }: TagAdvancedFilterProps) {
  const [conditions, setConditions] = useState<TagCondition[]>([newCondition()]);
  ...
  const updateCondition = useCallback((condId: string, patch: Partial<TagCondition>) => {
    setConditions((prev) =>
  ...
  const handleApply = useCallback(() => {
    ...
    const keyCondition = conditions.find((c) => c.field === "key" && c.value.trim() !== "");
    const tagKey = keyCondition?.value.trim();

    for (const cond of conditions) {
      if (!cond.value.trim() || cond.field === "key") {
        continue;
      }
      const value = cond.value.trim();
      const field = tagKey ? `tags.${tagKey}` : `tags.${cond.field}`;
      if (tagKey) {
        chips.push(createFilterChip(field, cond.op, value, "add_filter"));
      } else {
        chips.push(createFilterChip(`tags.${cond.field}`, cond.op, value, "add_filter"));
      }
    }
    onApply(chips);
    onClose?.();
          ...
          <div key={cond.id} style={{ marginBottom: 4 }}>
            ...
            <Space size={4} wrap>
              <Select
                size="small"
                value={cond.field}
                onChange={(v) => updateCondition(cond.id, { field: v as TagCondition["field"], op: "eq", value: "" })}
                style={{ width: 110 }}
                options={TAG_FIELDS}
              />
              <Select
                size="small"
                value={cond.op}
                onChange={(v) => updateCondition(cond.id, { op: v as TagOperator })}
                style={{ width: 90 }}
