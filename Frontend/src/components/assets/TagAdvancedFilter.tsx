...
export function resolveTagField(tagKey: string): string {
	switch (tagKey) {
		// Keep quick-filter and advanced-filter on the same canonical fields.
		case "priority":
		case "quality":
...
function resolveTagConditionField(cond: TagCondition, tagKey?: string): string | null {
	if (cond.field === "key") {
		return null;
	}
	if (cond.field === "value") {
		return tagKey ? resolveTagField(tagKey) : "tags.value";
	}
	return `tags.${cond.field}`;
}
...
export default function TagAdvancedFilter({
	...
	const handleApply = useCallback(() => {
		...
		if (tagKey && !hasNonKeyCondition) {
			...
			chips.push(createFilterChip("tags.key", "eq", tagKey, "add_filter"));
		}
		onApply(chips);
		onClose?.();
