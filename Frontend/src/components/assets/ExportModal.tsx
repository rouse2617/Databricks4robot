...
function buildAlgoSummary(algoResults: Record<string, string> | undefined): string {
...
function generateCsv(assets: Asset[]): string {
  ...
  const rows = assets.map((a) => {
    const values: string[] = [
      ...
      a.env ?? "",
      a.task ?? "",
      a.duration_sec != null ? String(a.duration_sec) : "",
      a.owner ?? "",
      a.reviewer ?? "",
...
async function fetchAllFiltered(
  ...
  const totalPages = Math.ceil(Math.min(totalFiltered, MAX_EXPORT_ROWS) / FETCH_PAGE_SIZE);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (format === "csv") {
        ...
        triggerDownload(content, `assets-${timestamp}.csv`, "text/csv;charset=utf-8");
      } else {
      ...
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        ...
        <div>
          ...
          <Radio.Group
            ...
            disabled={exporting}
          >
            <Radio value="current_page">当前页（{currentPageItems.length} 条）</Radio>
