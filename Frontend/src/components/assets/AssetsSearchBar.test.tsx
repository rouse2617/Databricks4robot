...
describe("AssetsSearchBar component", () => {
	...
	it("commits query on blur when draft is valid and differs from committed", () => {
		const onCommitQuery = vi.fn();
		render(
			<AssetsSearchBar
				searchMode="structured"
				draftText="env:warehouse"
				committedQueryText=""
				onDraftChange={vi.fn()}
				onCommitQuery={onCommitQuery}
				onModeChange={vi.fn()}
		...
		fireEvent.blur(screen.getByTestId("assets-search-input"));
		expect(onCommitQuery).toHaveBeenCalledTimes(1);
		expect(onCommitQuery).toHaveBeenCalledWith(
			"env:warehouse",
			expect.any(Array),
		);
	});

	it("does not commit on blur when draft matches committed", () => {
		const onCommitQuery = vi.fn();
		render(
			<AssetsSearchBar
				searchMode="structured"
				draftText="env:warehouse"
				committedQueryText="env:warehouse"
				onDraftChange={vi.fn()}
				onCommitQuery={onCommitQuery}
				onModeChange={vi.fn()}
			/>,
