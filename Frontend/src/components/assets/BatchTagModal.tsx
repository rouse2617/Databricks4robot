...
export default function BatchTagModal({
	...
	const handleExecute = useCallback(async () => {
		...
		msg.open({
			key: "batch-tag",
			type: "loading",
			content: `批量打标签执行中（0/${assetIds.length}）`,
			duration: 0,
		});
		...
		await runWithConcurrency(tasks, 5, () => {
			...
			msg.open({
				key: "batch-tag",
				type: "loading",
				content: `批量打标签执行中（${completed}/${assetIds.length}）`,
				duration: 0,
			});
		});
		...
		msg.open({
			...
			content: `批量打标签完成：成功 ${success}，跳过 ${skipped}，失败 ${failed}`,
			duration: 3,
		});
		onComplete({ success, skipped, failed });
	}, [selectedKey, tagValue, strategy, assetIds, onComplete, msg]);
				...
				{executing && (
					<div>
						...
						<Text type="secondary" style={{ fontSize: 12 }}>
							正在处理 {completed}/{assetIds.length}
						</Text>
					</div>
				)}
			</Space>
