...
import { Modal, Select, Input, Radio, Progress, Typography, Space, message } from "antd";
...
export default function BatchTagModal({
    ...
    const tasks = assetIds.map((assetId) => async () => {
      try {
        ...
        if (strategy === "skip") {
          ...
          if (asset.tags && asset.tags[selectedKey] !== undefined && asset.tags[selectedKey] !== "") {
            skipped++;
            return;
          }
        }
  ...
  return (
    ...
    >
      ...
      <div style={{ background: "#e6f7ff", border: "1px solid #91d5ff", borderRadius: 4, padding: "8px 12px", marginBottom: 16, fontSize: 13 }}>
      ...
      <Space direction="vertical" style={{ width: "100%" }} size={16}>
        ...
        <div>
          ...
          <Select
            ...
            loading={loading}
            value={selectedKey}
            onChange={(v) => { setSelectedKey(v); setTagValue(""); }}
        ...
        {selectedTag && (
          <div>
            ...
            {selectedTag.type === "enum" && selectedTag.values && selectedTag.values.length > 0 ? (
              <Select
                style={{ width: "100%" }}
                placeholder="选择标签值"
                value={tagValue || undefined}
                onChange={setTagValue}
                options={selectedTag.values.map((v) => ({ label: v, value: v }))}
              />
            ) : (
              <Input
                placeholder="输入标签值"
