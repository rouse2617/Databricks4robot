...
import type { FetchStatus, PreviewManifest } from "../../lib/assets/assetsDiscoveryTypes";
...
function renderPane(overrides: Partial<Parameters<typeof AssetQuickPreviewPane>[0]> = {}) {
  const props = {
...
describe("AssetQuickPreviewPane", () => {
  ...
  it("calls onCollapse when collapse button is clicked in expanded state", () => {
    ...
    const btn = screen.getByTitle("收起预览");
    fireEvent.click(btn);
    expect(fn).toHaveBeenCalledOnce();
  });
