...
import type { PipelineTemplate, Deployment } from "../../api/pipelineApi";
...
const mockTemplate = (overrides: Partial<PipelineTemplate> = {}): PipelineTemplate => ({
  id: "tmpl-001",
  name: "test-pipeline",
...
function renderDeployPanel(onEditTemplate?: (pipeline: PipelineTemplate["pipeline"]) => void) {
  return render(
    <MemoryRouter>
...
describe("DeployPanel", () => {
  ...
  it("loads and displays templates and deployments on mount", async () => {
    ...
    mockListDeployments.mockResolvedValue([
      mockDeployment({ id: "dep-001", pipelineName: "my-pipeline", status: "Succeeded" }),
    ]);
    renderDeployPanel();
  ...
  it("shows error toast when direct deploy fails", async () => {
    ...
    await waitFor(() => {
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining("部署失败"));
    });
  });
  ...
  it("opens asset modal via dropdown and deploys with asset ids", async () => {
    ...
    const deployBtn = document.querySelector(".ant-modal-footer .ant-btn-primary");
  ...
  it("deploys without asset IDs when none selected", async () => {
    ...
    expect(screen.getByTestId("mock-asset-picker").textContent).toContain("(none)");
  ...
  it("calls handleEditTemplate with onEditTemplate callback", async () => {
    const onEdit = vi.fn();
    const pipeline = { name: "test-pipeline", version: "1", nodes: [], edges: [] };
    mockListPipelines.mockResolvedValue([mockTemplate({ id: "tmpl-001", pipeline })]);
    mockListDeployments.mockResolvedValue([]);
    mockGetPipeline.mockResolvedValue(mockTemplate({ id: "tmpl-001", pipeline }));
  ...
  it("shows error when load template fails on edit", async () => {
    ...
    const { message } = await import("antd");
    await waitFor(() => {
      expect(message.error).toHaveBeenCalledWith(expect.stringContaining("加载模板失败"));
    });
  });
