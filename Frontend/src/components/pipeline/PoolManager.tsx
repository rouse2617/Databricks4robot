...
import { listExecutionTargets, type ExecutionTarget } from "../../api/pipelineApi";
...
export default function PoolManager() {
		...
		try {
			...
			if (editTarget) {
				await fetch(`/api/v1/execution-targets/${editTarget.id}`, {
					method: "PUT",
					headers: { "Content-Type": "application/json", "X-Databrew-Token": "dev-token" },
	...
	const cpuPct = (q?: QuotaInfo) => {
		if (!q) return 0;
		return Math.min(100, Math.round((parseFloat(q.cpu.used) / parseFloat(q.cpu.hard)) * 100));
	};
	...
	const columns = [
		{
			...
			render: (name: string, r: ExecutionTarget) => (
				<Text strong>{name}{r.isDefault ? <Tag color="blue" style={{ marginLeft: 6 }}>默认</Tag> : null}</Text>
			),
		},
		{
			title: "命名空间",
			...
			key: "ns",
			width: 160,
			render: (ns: string) => <Text style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>{ns}</Text>,
		},
		{
			title: "CPU",
			key: "cpu",
			...
			render: (_: unknown, r: ExecutionTarget) => {
				...
				return (
					<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
						...
						<Text style={{ fontSize: 11, fontFamily: "var(--font-mono)", color, minWidth: 50 }}>
			...
			render: (_: unknown, r: ExecutionTarget) => {
				...
				return (
					<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
						...
						<Text style={{ fontSize: 11, fontFamily: "var(--font-mono)", color, minWidth: 60 }}>
		...
		},
		{
			...
			width: 80,
			render: (s: string) => (
				<Tag color={s === "available" ? "green" : "default"}>{s === "available" ? "可用" : "不可用"}</Tag>
			),
		},
		{
			key: "actions",
			...
			render: (_: unknown, r: ExecutionTarget) =>
				r.isDefault ? null : (
					<Space size={4}>
						<Tooltip title="编辑"><Button type="text" size="small" icon={<EditOutlined />} onClick={() => openEdit(r)} /></Tooltip>
						<Tooltip title="删除">
							<Button type="text" size="small" danger icon={<DeleteOutlined />}
								onClick={() => Modal.confirm({
									...
									onOk: async () => {
										try {
											await fetch(`/api/v1/execution-targets/${r.id}`, { method: "DELETE", headers: { "X-Databrew-Token": "dev-token" } });
											message.success("已删除");
											fetchData();
	...
	return (
		<>
			<Card size="small" title="资源池管理" extra={
				<Space size={8}>
				...
				<div style={{ marginTop: 12, color: "var(--gray-400)", fontSize: 12 }}>
					<Typography.Text type="secondary">
						资源池对应 K8s 命名空间，部署流水线时选择资源池即提交 Workflow 到对应命名空间，受 ResourceQuota 约束。
					</Typography.Text>
				</div>
			</Card>
			...
			>
				<Form form={form} layout="vertical" style={{ marginTop: 16 }}>
					...
					<Form.Item name="namespace" label="K8s 命名空间" rules={[{ required: true, message: "请输入命名空间" }]}>
