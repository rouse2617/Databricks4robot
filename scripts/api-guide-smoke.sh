...
get() {
	local name="$1" path="$2"
	local raw
	raw=$(curl -sS --max-time 25 -w "\n%{http_code}" "${HDR[@]}" "${BASE}${path}" 2>/dev/null) || raw=$'\n000'
...
get_report_or_skip() {
	local raw
	raw=$(curl -sS --max-time 25 -w "\n%{http_code}" "${HDR[@]}" "${BASE}/api/v1/lakehouse/report" 2>/dev/null) || raw=$'\n000'
	RESP_CODE=$(echo "$raw" | tail -n1)
	RESP_BODY=$(echo "$raw" | sed '$d')
...
post() {
	...
	local raw
	raw=$(curl -sS --max-time 30 -w "\n%{http_code}" -X POST "${HDR[@]}" "$BASE$path" -d "$data" 2>/dev/null) || raw=$'\n000'
	RESP_CODE=$(echo "$raw" | tail -n1)
	RESP_BODY=$(echo "$raw" | sed '$d')
	if [[ "$RESP_CODE" =~ ^2 ]]; then ok "$name"; else bad "$name"; fi
	echo "$RESP_BODY"
...
get "lakehouse/tag-timeline" "/api/v1/lakehouse/tag-timeline?tag_key=quality"
get "lakehouse/quality-distribution" "/api/v1/lakehouse/quality-distribution?window=30d"
get "lakehouse/customer-replay" "/api/v1/lakehouse/customer-replay?customer_id=urn:grace:customer:A"
...
LIST_RAW=$(curl -sS --max-time 20 -X POST "${HDR[@]}" "${BASE}/api/v1/queries/run" -d '{"schema_version":"v1","mode":"structured","scope":{"resource":"assets"},"page":{"page":1,"page_size":1}}' || true)
