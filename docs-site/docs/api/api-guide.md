...
> OpenAPI 规范: [`api/openapi.yaml`](https://github.com/CyberOrigin2077/cyber-databrew/blob/dev/api/openapi.yaml)
...
仓库脚本：[`scripts/api-guide-smoke.sh`](https://github.com/CyberOrigin2077/cyber-databrew/blob/dev/scripts/api-guide-smoke.sh)（仓库根目录执行）。Makefile：`make api-guide-smoke`（默认本地 `BASE` / `TOKEN`，可用变量覆盖）。
...
实现：将 [`scripts/api-guide-smoke.sh`](https://github.com/CyberOrigin2077/cyber-databrew/blob/dev/scripts/api-guide-smoke.sh) 同步为 ConfigMap，并 apply Job [`deploy/k8s/jobs/api-guide-smoke-job.yaml`](https://github.com/CyberOrigin2077/cyber-databrew/blob/dev/deploy/k8s/jobs/api-guide-smoke-job.yaml)。BigQuery 未配置时，部分 Lakehouse 接口会为 **503** / **WARN**，与 api-guide「1.0 未上线」一致。
