...
- [x] [Frontend] Sort the batch job list by `createdAt` newest first by default and keep the creation-time column user-sortable.
- [x] [Frontend] Make batch detail polling status-aware: running jobs poll at 5s, paused jobs at 30s, terminal jobs stop polling.
- [x] [Frontend] Buffer realtime workflow-log SSE lines before React state updates, cap retained client log lines at 5000, and virtualize the log viewer rows.
- [x] [Frontend] Add a realtime-log connection grace state so EventSource streams that stay open without events show "connected, waiting for log events" instead of remaining stuck on "connecting".
- [x] [Frontend] Add node runtime terminal controls backed by backend terminal-session APIs, including command selection, WebSocket attach, stdout/stderr append, backend error-code display, and session reopen after exit.
- [x] [Frontend] Hide the explicit "no estimated cost" warning when billing config is unavailable so node status, logs, Pod diagnostics, and monitoring stay prominent.
- [x] [scripts] Add `scripts/smoke-pipeline-runtime-baseline-dev.sh` for fixed sleep/terminal, log-spam, and resource-request runtime smoke baselines.
...
- [x] [Frontend] 2026-06-20 targeted runtime stability checks: `npx biome check` on touched workflow/batch/log files; `vitest run src/pages/BatchJobList.test.tsx src/pages/useWorkflowDetail.test.tsx src/pages/workflowLogView.test.ts src/components/pipeline/WorkflowNodeDetailPanel.test.tsx src/pages/BatchJobDetailPage.test.tsx src/pages/WorkflowDetailPage.test.tsx` passed (6 files, 56 tests); `tsc -b --pretty false` passed; `npm run build` passed with existing CSS minify/chunk-size warnings; `git diff --check` and `bash -n scripts/smoke-pipeline-runtime-baseline-dev.sh` passed.
- [x] [Chrome DevTools MCP] 2026-06-20 local UI validation on `http://127.0.0.1:5180`: batch job list at `/pipeline?tab=executions&executionView=batch` shows creation-time default descending order (`18:02:04`, `17:55:31`, `09:27:23`); execution detail `pipeline-1781872251305-7457a0` runtime drawer loads Pod diagnostics and resource request/limit snapshot, shows terminal disabled policy reason, hides the explicit no-estimated-cost warning, and realtime logs leave the indefinite "connecting" state after the grace interval.
- [x] [repo] Pre-commit after frontend dev verification approval: `pre-commit run --all-files` was blocked before hooks by the local Homebrew Python 3.14 `pyexpat` import error; scoped fallback `GOPROXY=https://goproxy.cn,direct uv tool run pre-commit run --files $(cat /tmp/cyb2224-runtime-stability-files.txt)` passed for the intended non-`site/` runtime-stability files.
...
- [x] Frontend Cloud Run dev deploy for runtime stability and batch sorting.
  - Image: `us-central1-docker.pkg.dev/green-valley-442103/cyber-databrew-images/cyber-databrew-frontend:48a56fc-cyb2224-runtime-stability-20260620003811`
  - Latest alias: `us-central1-docker.pkg.dev/green-valley-442103/cyber-databrew-images/cyber-databrew-frontend:cloudrun-dev-latest`
  - Revision: `cyber-databrew-frontend-dev-00390-mlt`
  - Cloud Run URL: `https://cyber-databrew-frontend-dev-wtttm6suaq-uc.a.run.app`
  - Public URL: `https://cyber-databrew-dev.cyberorigin.ai`
- [x] Chrome DevTools MCP on public dev domain after revision `00390-mlt`: batch job list at `/pipeline?tab=executions&executionView=batch` loads bundle `BatchJobList-C7msNsDN.js` and defaults creation time newest first (`2026-06-19 18:02:04`, `2026-06-19 17:55:31`, `2026-06-19 09:27:23`). Evidence: `openspec/changes/CYB-2224-runtime-mount-resources/cf-batch-list-created-desc-20260620.png`.
- [x] Chrome DevTools MCP on public dev domain after revision `00390-mlt`: completed batch detail `029687f8-5f43-4288-9bd1-42aa1dbad30d` loads summary/node/current-page run data without terminal-state polling, and execution detail `pipeline-1781832426939-e9e5fe` opens workflow logs, transitions realtime log state to `已结束`, shows resource request/limit snapshot, and displays terminal disabled reason `Pod 终端未启用，请在执行目标配置中开启`; historical Pod lookup 404 is handled as `Pod 诊断数据不可用` while `/resources` returns 200.
...
- [x] Backend dev deploy after final `dev` cherry-pick: Cloud Build `fd8e0356-6f0d-4c87-9bee-c86d5a428b2d` built image `us-central1-docker.pkg.dev/green-valley-442103/cyber-databrew-images/cyber-databrew-backend:1043eba-cyb2224-terminal-dev-20260619230926`; Cloud Run revision `cyber-databrew-backend-dev-00947-cnh` serves 100% traffic; `/readyz` returned healthy; env includes `PIPELINE_TEMPLATE_TOLERATIONS_JSON` and `PIPELINE_RUNTIME_SECRET_RESOURCES_JSON`, and does not include `PIPELINE_TARGET_RESOURCE_CEILINGS_JSON`.
- [x] Backend terminal smoke after revision `00947-cnh`: public dev domain `GET /api/v1/execution-targets` returned `terminal=true` for `default` and `video-proc-dev`; test run `96a99847-1074-45cb-8965-6767dc922072` / workflow `cyb2224-terminal-final-231444-4c5a9d` reached node `cyb2224-terminal-final-231444-4c5a9d-1988065096` with `execEnabled=true`; terminal sessions defaulted `containerName=main`, attached successfully, returned status frame `attached` plus `env` stdout, and ended with no error. The smoke workflow was stopped and finished as `Failed / Stopped with strategy 'Stop'`.
